...
def show_definition(appellation):
    ...
    result = ("\n" + appellation + ":\n" + mini_dict[appellation.lower()][0] + "\nsource: " + mini_dict[appellation.lower()][1])
...
def show_all():
    ...
    #i don't fully understand why 'sorted' returns only keys in alphabetical order
        ...
        with open ('dictionary.csv' , 'r') as db:
            ...
            for org_key , org_definition , org_source in reader:
                if org_key.lower() == key.lower():
                    ...
                    result = False
    except FileNotFoundError: #quite obvious that when there is no file, our def is not in it :)
        result = False
    return result
...
while True: #program main loop.
    ...
    try:
        ...
        if action == "1": #searching by appellation
        ...
        elif action == "3": #show all avaible appelations in alphabetical order
            ...
            if show_by_letter(letter) == []:
                print ("No appelations starting with " + letter.upper() + ".\nUse menu to add some.")
            else:
    ...
    # KI and EOFE for additional program quiting, FNFE allows to create a new DB
    except (KeyboardInterrupt, EOFError):
        sys.exit("\nYou have exited a dictionary")
    except FileNotFoundError:
        print ("\nMissing database file. Add new definition to make new one, or check the program folder")
